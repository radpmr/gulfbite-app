...
# Use CPU inference here so local runs and Streamlit deployment behave consistently.
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"

# iPhones often upload HEIC/HEIF files, so register support when the package is available.
try:
    import pillow_heif
...
# App settings and the lookup tables used by the recognition pipeline.
...
# Dishes in these sets get an extra check because the CNN can confuse them visually.
TRIGGER_SET = {
    "07_ouzi",
    "01_machboos",
    "09_jisheed",
...
# Return the user-facing reason for dishes that are visually easy to confuse.
...
# Not every YOLO feature is reliable enough to override the CNN, so keep that decision explicit.
FEATURE_RELIABILITY = {
...
# Recipe weights are approximate serving components used for the nutrition estimate.
DISH_RECIPES = {
...
# Limit the second-stage check to dishes that genuinely look similar.
...
# Model loading and inference helpers. These are kept separate from the UI
# so the recognition logic is easier to test and change later.

@st.cache_resource
# Load whichever model files are available. Missing optional pieces should not break the whole app.
def load_models():
...
# Run the first-pass classifier and keep the confidence values needed for rejection checks.
...
# YOLO is only used when the first prediction needs an extra visual clue.
...
# Draw the detected food features on a copy of the image for the verification screen.
...
# Convert detected visual features back into the most plausible Gulf dish.
...
# Scale the recipe estimate by the portion selected by the user.
...
# Most of the visual styling lives here. Streamlit needs a few targeted CSS
# overrides to make the app feel closer to a compact mobile interface.

# Inject the app theme in one place so screen code stays focused on layout and behaviour.
def inject_theme():
    st.markdown(
        """<style>
...
# Keep the same branding at the top of every main screen.
...
# Bottom navigation is intentionally simple: Home, Menu and Scan.
...
# Show progress through Scan -> Verify -> Portion -> Macros without adding another navigation layer.
...
# Mark the next rerun so the new stage starts at the top of the screen.
...
# Favourite tiles use a tiny query parameter so they can jump straight to the matching Menu dish.
...
# Keep upload handling in one helper because the same file types and validation rules are reused.
...
# Build the Menu screen from the category chips, dish picker and selected-dish profile.
...
# Present the nutrition estimate in a compact visual summary rather than a plain table.
...
# Session state is the simple router for this prototype. It keeps the current
# screen and recognition result available between Streamlit reruns.
...
# Clear the current recognition result when the user starts over.
...
# Load the models once and reuse them across reruns. This avoids paying the
# model-loading cost every time the user clicks a control.
...
# Onboarding screen shown before the user enters the main app.
...
# Main app shell: Home, Menu and Scan share the same header and bottom nav.
...
@media (max-width: 768px) {
    ...
    st.markdown(
        """
        <style>
        /* One shared dish-picker skin for Menu, Verify and Result.
           Behaviour stays in Streamlit session state; there are no links or query params. */
        .stApp .st-key-menu_clean_dish_picker,
        .stApp .st-key-confirm_clean_dish_picker,
        .stApp .st-key-result_clean_dish_picker {
            width: 100% !important; margin: 0 !important; padding: 0 !important;
            border: 0 !important; background: transparent !important; box-shadow: none !important;
        }

        /* Closed selector — matches the earlier clean cream/gold picker. */
        .stApp .st-key-menu_clean_dish_picker .st-key-menu_clean_dish_picker_trigger button,
        ...
        .stApp .st-key-result_clean_dish_picker .st-key-result_clean_dish_picker_trigger button {
            ...
            padding: 0 14px !important; border: 1px solid #D99A1B !important; border-radius: 14px !important;
            background: linear-gradient(180deg,#FFFEFB 0%,#FFF8E9 100%) !important;
            color: #A66D05 !important; box-shadow: none !important; transform: none !important;
            font-size: .92rem !important; font-weight: 600 !important; text-align: left !important;
            justify-content: space-between !important;
        }
        .stApp .st-key-menu_clean_dish_picker .st-key-menu_clean_dish_picker_trigger button:hover,
        .stApp .st-key-confirm_clean_dish_picker .st-key-confirm_clean_dish_picker_trigger button:hover,
        .stApp .st-key-result_clean_dish_picker .st-key-result_clean_dish_picker_trigger button:hover {
            background: #FFF3D7 !important; border-color: #C88712 !important; color: #8D5C04 !important;
        }

        /* Option area has no outer white shell/card. */
        ...
        .stApp [data-testid="stVerticalBlockBorderWrapper"]:has(.st-key-result_clean_dish_picker_options) {
            margin: 4px 0 0 !important; padding: 0 !important; border: 0 !important; border-radius: 0 !important;
        ...
        /* Flat text-only rows. Global app button styles are deliberately overridden here. */
        .stApp .st-key-menu_clean_dish_picker_options button,
        .stApp .st-key-confirm_clean_dish_picker_options button,
        .stApp .st-key-result_clean_dish_picker_options button {
            width: 100% !important; min-height: 34px !important; height: 34px !important; margin: 0 !important;
            padding: 0 10px !important; border: 0 !important; border-radius: 7px !important;
            background: transparent !important; color: #2B241C !important; box-shadow: none !important;
            transform: none !important; justify-content: flex-start !important; text-align: left !important;
            font-size: .78rem !important; font-weight: 550 !important;
        }
        .stApp .st-key-menu_clean_dish_picker_options button:hover,
        .stApp .st-key-confirm_clean_dish_picker_options button:hover,
        .stApp .st-key-result_clean_dish_picker_options button:hover {
            background: #FFF3D7 !important; color: #241B12 !important; border: 0 !important; box-shadow: none !important;
        }
        .stApp .st-key-menu_clean_dish_picker_options button *,
        .stApp .st-key-confirm_clean_dish_picker_options button *,
        .stApp .st-key-result_clean_dish_picker_options button * {
        ...
        @media (max-width: 768px) {
            ...
            .stApp .st-key-confirm_clean_dish_picker .st-key-confirm_clean_dish_picker_trigger button,
            .stApp .st-key-result_clean_dish_picker .st-key-result_clean_dish_picker_trigger button {
                min-height: 46px !important; height: 46px !important; font-size: .88rem !important;
            }
            .stApp .st-key-menu_clean_dish_picker_options button,
            .stApp .st-key-confirm_clean_dish_picker_options button,
            .stApp .st-key-result_clean_dish_picker_options button {
                min-height: 32px !important; height: 32px !important; font-size: .75rem !important;
            }
        }
        </style>
        """,
...
# After recognition, let the user check the predicted dish before moving on.
...
# Portion selection is kept separate because the nutrition estimate depends on it.
...
# Final nutrition view: calories, macros and the technical pipeline details.
